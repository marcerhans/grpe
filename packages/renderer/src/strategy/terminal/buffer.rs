pub mod pixel {
    ...
    //     Value::Empty.value(),
    // ];
    pub const VALUE_MAX_LEN: usize = 3;
    pub const EMPTY: [u8; VALUE_MAX_LEN] = *Value::Empty.value();
    ...
    impl Pixel {
        ...
        pub fn value(&self) -> &[u8; 3] {
            self.slice[(VALUE_MAX_LEN - 3)..VALUE_MAX_LEN]
                .try_into()
                .unwrap()
        }

        pub fn set_value(&mut self, value: Value) {
            self.slice[(VALUE_MAX_LEN - 3)..VALUE_MAX_LEN].copy_from_slice(value.value());
        }
    ...
    impl Value {
        pub const fn value(&self) -> &[u8; 3] {
            match self {
                Value::Upper => &[0xE2, 0x96, 0x80], // '\u{2580}', ▀
                Value::Lower => &[0xE2, 0x96, 0x84], // '\u{2584}', // ▄
                Value::Full => &[0xE2, 0x96, 0x88],  // '\u{2588}',  // █
                Value::Empty => &[0xE2, 0x80, 0x83], // '\u{2003}, // "EM Space" - Just some whitespace character with 3 bytes.
            }
        }
...
impl TerminalBuffer {
    ...
    pub fn new(resolution: &(u64, u64)) -> Self {
        ...
        let mut data = vec![0 as u8; data_len];
        ...
        while meta.len() < meta_len {
            ...
            if col == meta_dimensions.0 {
                // Go to next row.
                data[(col + row * resolution.0 as usize) * pixel::VALUE_MAX_LEN + row] = b'\n';
                row += 1;
                col = 0;
            }
